...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      ...
      // We should clear the storage to prevent infinite 401 loops
      localStorage.removeItem('auth-storage');
      // Note: We can't easily call logout() here as it's a hook, 
      // but clearing storage will cause the next page load/state check to see the user as logged out.
      // Alternatively, we can trigger a window reload or use a custom event.
