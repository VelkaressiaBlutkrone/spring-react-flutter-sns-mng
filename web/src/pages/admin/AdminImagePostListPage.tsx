/**
 * 관리자 이미지 게시글 목록 페이지 (TASK_WEB Step 7).
 * GET /api/admin/image-posts. 페이징·검색·공지 토글·수정·삭제.
...
export default function AdminImagePostListPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['admin', 'image-posts', page, keyword],
    queryFn: () => adminImagePostsApi.list({ page, size, keyword: keyword || undefined }),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => adminImagePostsApi.delete(id),
    onSuccess: () => {
  ...
  if (isLoading) return <AppLayout title="관리자 - 이미지 게시글"><div className="text-slate-600">로딩 중...</div></AppLayout>;
  if (error) return <AppLayout title="관리자 - 이미지 게시글"><div className="text-red-600">목록을 불러오지 못했습니다.</div></AppLayout>;
          ...
          content.map((post) => (
            <li key={post.id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow">
              <div className="flex gap-4">
                <Link to={`/image-posts/${post.id}`} className="shrink-0">
                  <img
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Link to={`/image-posts/${post.id}`} className="font-semibold text-slate-900 hover:underline">
                      {post.title}
                    </Link>
                  </div>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  <Link
                    to={`/admin/image-posts/${post.id}/edit`}
...
function NoticeToggleButton({ postId, notice }: { postId: number; notice: boolean }) {
  ...
  const mutation = useMutation({
    mutationFn: () => adminImagePostsApi.toggleNotice(postId, !notice),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'image-posts'] });
    },
  });
